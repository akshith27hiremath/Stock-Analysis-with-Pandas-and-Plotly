...
def rsigenerate(df):
    ...
    rsi_fig.update_layout(title='Relative Strength Index (RSI)', xaxis_title='Date', yaxis_title='RSI Value',yaxis=dict(range=[0, 100]), template='plotly_dark')
...
def macdgenerate(df):
    ...
    fig.update_layout(title='MACD, Signal Line, and Histogram',xaxis_title='Date',yaxis_title='Value',barmode='relative',  height=600,template='plotly_dark')
    return(fig)

def bbandsgenerate(df, closedf):
    fig = go.Figure()
    ...
    fig.update_layout(title='Bollinger Bands with Close Price',xaxis_title='Date',yaxis_title='Price',height=600,template='plotly_dark')
    return(fig)

def ichimokucurrentgenerate(df, closedf):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Date'],y=closedf['Close'],name='Close Price',line=dict(color='blue')))
    ...
    fig.update_layout(title='Ichimoku Cloud with Future Clouds',xaxis_title='Date',yaxis_title='Price',height=600,template='plotly_dark')
    return(fig)

def ichimokufuturegenerate(df, closedf):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Date'],y=closedf['Close'],name='Close Price',line=dict(color='blue')))
    ...
    fig.update_layout(title='Ichimoku Cloud with Current Clouds',xaxis_title='Date',yaxis_title='Price',height=600,template='plotly_dark')
    return(fig)
...
#     )
#     fig.update_layout(
#         title='Close Price with Parabolic SAR',
#         xaxis_title='Date',
#         yaxis_title='Price',
#         height=600,
#         template='plotly_dark'  # Optional: Choose a template you like
#     )
...
for stock in output_dataframes:
    ...
    os.mkdir(save_dir)
    rsigraph = rsigenerate(rsimacd)
    rsigraph.write_image(f"{save_dir}rsi.png")
    bbandsgraph = bbandsgenerate(bbands, rsimacd)
    bbandsgraph.write_image(f"{save_dir}bbands.png")
    macdgraph = macdgenerate(rsimacd)
    macdgraph.write_image(f"{save_dir}macd.png")
    ichimokucurrentgraph = ichimokucurrentgenerate(ichimokucurrentdf, rsimacd)
    ichimokucurrentgraph.write_image(f"{save_dir}ichimokucurrent.png")
    ichimokufuturetgraph = ichimokucurrentgenerate(ichimokufuturedf, rsimacd)
